...
def train(j,model, train_loader, val_loader, epoch, num_epochs, loss_function, optimiser, scheduler, savename, highest_iou):
    model.train()
    losses = list()
    gpu2 = 'cuda:0'
    gpu1 = 'cuda:1'
    ...
    for data, target in loop:
        ...
        if j%9 == 0:
            torch.save(model.state_dict(), savename[:-3]+str(j)+'.pt')
            torch.save(optimiser.state_dict(), savename2[:-3]+str(j)+'.pt')

    return max_iou

class CityscapesDataset(data.Dataset) :
    def __init__(self, root = '/home/himanshu/dl/dataset/train_sih/', image_path = 'images', transform = None, size = 4) :
        self.img_list, self.mask_list = CityscapesDataset.get_filenames(root, image_path)
        self.transform = transform
        self.inshape = [1000 // size, 1000 // size]
    ...
    def get_filenames(path, image_path) :
        ...
        for filename in os.listdir(os.path.join(path, image_path)):
            ...
            mask_list.append(os.path.join(path, 'masks', filename[: -4]) + '.png')
